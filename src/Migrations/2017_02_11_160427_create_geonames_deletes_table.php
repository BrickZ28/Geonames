...
class CreateGeonamesDeletesTable extends Migration {
    ...
    public function up() {
        Schema::create( 'geonames_deletes', function ( Blueprint $table ) {
            ...
            $table->date( Geonames\Models\GeonamesDelete::date );           // The date that this record was removed
	        // from the geonames database.
            $table->integer( Geonames\Models\GeonamesDelete::geonameid );   // geonameid         : integer id of
	        // record in geonames database
            $table->string( Geonames\Models\GeonamesDelete::name,
                            200 );    // name              : name of geographical point (utf8) varchar(200)
            $table->string( \Geonames\Models\GeonamesDelete::reason,
                            255 );   // The reason that this record was deleted.
            $table->timestamps();           // Laravel's created_at and updated_at timestamp fields.

            $table->unique( [
                                \Geonames\Models\GeonamesDelete::geonameid,
                                \Geonames\Models\GeonamesDelete::date,
                            ] );
        } );
